    ...
    public void simulateOneStep()
    {
        ...
        for (int i = 0; i < field.getDepth(); i++) {
            for (int j = 0; j < field.getWidth(); j++) {
                if (field.getObjectAt(i, j) instanceof Rabbit)
                    ((Rabbit) field.getObjectAt(i, j)).run();
                else if (field.getObjectAt(i, j) instanceof Fox)
                    ((Fox) field.getObjectAt(i, j)).hunt();
            }
        }
    ...
    private void populate()
    {
        ...
        for (int i = 0; i < field.getDepth(); i++) {
            for (int j = 0; j < field.getWidth(); j++) {
                ...
                if (rand <= 0.01)
                    field.place(new Fox(field, new Location(i, j)), i, j);
                if(field.getObjectAt(i, j) == null)
                    if(rand >= 0.1 && rand <= 0.17)
                        field.place(new Rabbit(field, new Location(i, j)), i, j);
            }
        }
    }
}